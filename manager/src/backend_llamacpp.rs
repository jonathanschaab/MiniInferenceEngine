    ...
    pub fn new() -> Result<Self, String> {
        ...
        std::thread::spawn(move || {
            ...
            while let Some(cmd) = rx.blocking_recv() {
                match cmd {
                    EngineCommand::LoadModel { config, status, strategy, required_ctx, reply } => {
                        ...
                        let compute_margin: u64 = LLAMA_CPP_COMPUTE_MARGIN_BYTES;
                            ...
                            let offload_pct = 1.0 - (n_gpu_layers as f32 / config.num_layers.max(1) as f32);
                            if offload_pct > 0.0 {
                                let offloaded_bytes = (weights_vram_cost_est as f32 * offload_pct) as i64;
                            ...
                            s.set_model_vram(config.id.clone(), "LlamaCpp".to_string(), true, "Active".to_string(), weights_vram, kv_vram, compute_margin);
...
impl InferenceBackend for LlamaCppEngine {
    async fn load_model(&mut self, config: &ModelConfig, status: Arc<Mutex<EngineStatus>>, strategy: &MemoryStrategy, required_ctx: usize) -> Result<usize, String> {
        ...
        let (final_ctx_len, offload_pct) = rx.await.map_err(|_| "Engine thread dropped reply".to_string())??;
    ...
    async fn generate_text(&mut self, prompt: &str, params: &GenerationParameters) -> Result<(String, u128), String> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        self.command_tx.send(EngineCommand::GenerateText {
            prompt: prompt.to_string(),
            params: params.clone(),
            reply: tx,
        }).map_err(|_| "Engine thread died".to_string())?;
        
        rx.await.map_err(|_| "Engine thread dropped reply".to_string())?
    }
